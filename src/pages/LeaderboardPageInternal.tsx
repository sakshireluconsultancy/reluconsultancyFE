...
export const CountdownTimer: React.FC<{ end: Date }> = ({ end }) => {
  ...
  const boxStyle =
    "flex flex-col items-center justify-center min-w-[3rem] p-1 lg:p-1 bg-white rounded-lg border border-blue-500 shadow-md text-blue-700 font-bold text-sm";

  return (
    <div className="flex items-center justify-center lg:gap-2 md:gap-1 gap-2">
...
const PastMonthWinnerShimmer = () => (
  <div className="relative bg-gradient-to-br from-white via-blue-50 to-indigo-100 rounded-3xl p-8  border border-white/50 backdrop-blur-sm animate-pulse">
...
const LeaderboardPageInternal: React.FC = () => {
  ...
  const fetchPastMonthData = async () => {
    try {
      setIsLoading(true);
      const internalPastMonth = await postAPI(
        "monthly-external-internal/",
        { month: month, participants: "All_participants" }
      );

      // setPastMonthData(internalPastMonth);
      setPastMonthData(internalPastMonth || []);
    } catch (error) {
  ...
  const fetchLeaderboardData = async () => {
    try {
      ...
      const internalResponse = await postAPI(
      ...
      setLeaderboardData(internalResponse || []);
    } catch (error) {
                  ...
                  {podium.map((p: LeaderboardItem, i: number) => {
                    return (
                      ...
                      >
                        {i === 0 && (
                          <span
                            className="absolute top-[-10%] z-1 h-[150px] w-full block bg-cover bg-center hidden "
                            style={{ backgroundImage: `url(${winner})` }}
                          />
                        )}
              ...
              {isLoading ? (
                ...
                <div className=" relative z-10 bg-gradient-to-br from-blue-50 via-blue-100 to-indigo-100 rounded-3xl p-8 border border-slate-50 backdrop-blur-md ">
                  ...
                  <div className="relative mt-4 lg:mt-0 flex-col md:flex-row flex items-center justify-between ">
                    <p className="text-xl  font-bold text-hpBlue capitalize">
...
export default LeaderboardPageInternal;
